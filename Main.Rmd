...
Part (b) is completed by constructing event time relative to each community’s flood year, estimating a TWFE event-study regression with fixed effects, and plotting the coefficients with confidence intervals. Interpretation focuses on dynamics before and after the flood and on appropriate clustering choices.
...
## Step B.2: Construct each community’s flood year and merge it back to the panel
...
This chunk creates a treated indicator and the relative year variable𝑟= year − flood_year for treated communities, plus quick checks of the resulting event-time support.
...
This chunk defines the event-study window, bins extreme leads and lags, and creates a factor variable with the correct reference period (r = −1). 
...
es_plot <- ggplot(es_plot_data, aes(x = rel_year, y = estimate)) +
  ...
  labs(
    ...
    title = "Figure 1. Event Study: Flood and Insurance Take-Up"
  ) +
...
This chunk applies the Goodman-Bacon decomposition to the TWFE DiD estimate in order to decompose it into all underlying 2×2 comparisons (treated vs never-treated, early vs late, and late vs early) and to record the weight and contribution of each comparison type.

```{r}

## Step C.3: Goodman–Bacon decomposition of the TWFE estimate (bacondecomp)
# This chunk runs the Goodman–Bacon decomposition of the TWFE regression ln_policies ~ post.
...
kable(
  ...
  caption  = "Table 1. Goodman–Bacon Decomposition of the TWFE Estimate",
  align    = c("l", "c", "c", "c"),
  booktabs = TRUE
) %>%
  kable_styling(
...
Recent work on staggered difference-in-differences designs shows that two-way fixed effects (TWFE) and event-study estimators can produce misleading estimates when treatment timing is staggered and treatment effects are dynamic. As emphasized by Baker et al. (2022), the TWFE estimand is generally a weighted average of heterogeneous 2×2 comparisons, including comparisons that use already-treated units as controls for later-treated units. When treatment effects evolve over time—as is plausible in Gallagher’s (2014) setting, where households update beliefs about flood risk after experiencing a flood—such “treated-as-control” comparisons need not recover a meaningful average treatment effect on the treated (ATT).

The Goodman–Bacon decomposition reported in Table X shows that about 71.7% of the identifying weight in the TWFE estimate comes from clean comparisons between treated and never-treated communities, which imply an average effect of 0.1147. However, a nontrivial share of weight is placed on comparisons among treated cohorts. In particular, “late-to-early” comparisons—where later-treated units are compared to already-treated units—receive about 9.98% of the total weight and imply an ATT of 0.2102 (Table X). This estimate is substantially larger than the effect implied by treated-versus-untreated comparisons, consistent with the presence of dynamic treatment effects that grow with time since treatment.

As shown in Table X, late-to-early comparisons contribute approximately 0.0210 to the overall TWFE estimate of 0.1211, accounting for roughly 17% of the aggregate effect. Thus, a meaningful portion of the TWFE estimate is driven by treated-as-control comparisons that are precisely those highlighted in the recent literature as potentially problematic under staggered adoption with dynamic effects. In the context of flood-insurance take-up, this suggests that Gallagher’s TWFE/event-study estimates partly reflect dynamic learning effects being re-weighted across cohorts, rather than a single stable ATT, and should therefore be interpreted with caution.
...
This chunk constructs the unit-level flood timing variable (flood_year) from the flood-year indicator (hityear), then defines the time-varying treatment indicator (treated_it) and the untreated/treated observation flags (Ω₀ and Ω₁) used in Steps (1)–(3).

```{r}

# Construct unit-level flood timing (flood_year) and then define treated_it and Ω0/Ω1 at the observation level
...
# Ω0: untreated observations (never-treated units and pre-treatment periods)
in_omega0 = ifelse(treated_it == 0L, 1L, 0L),

# Ω1: treated observations (post-treatment periods for treated units)
...
cat("\nSanity check C: Every observation classified as either Ω0 or Ω1 (should be all 1s)\n")
print(table(ps2$in_omega0 + ps2$in_omega1))

cat("\nSanity check D: treated_it takes only values 0 or 1\n")
print(table(ps2$treated_it))

cat("\nSanity check E: treated_it equals Ω1 by construction (cross-tab)\n")
...
This chunk estimates the unit and time fixed effects using only untreated observations (Ω₀), which will later be used to impute untreated potential outcomes for treated observations.

```{r}

# Estimate unit and time fixed effects using only untreated observations (Ω0)
...
This chunk constructs the imputed untreated potential outcome Y0_hat_it = lambda_hat_i + gamma_hat_t, and then computes the imputed treatment effect tau_hat_it = Y_it − Y0_hat_it.
...
This chunk estimates the dynamic treatment effects by regressing the imputed treatment effects (tau_hat) on relative-time indicators, normalizing event time −1 as the reference period (implemented by including Ω₀ in the regression with tau_hat = 0 for untreated observations).

```{r}

# Prepare event-study dataset: set tau_hat = 0 for Ω0 and keep imputed tau_hat for Ω1
...
## Step D.6: Publication-ready imputation event-study plot (−6 to +9 window)

This chunk produces a report-ready imputation-based event-study figure, restricting the displayed event-time window to −6 through +9 for a clean and defensible presentation.
...
es_imp_plot <- ggplot(es_imp_plot_data, aes(x = rel_year, y = estimate)) +
  ...
  labs(
    ...
    title = "Figure 2. Dynamic Effects of Flood Experience on Insurance Take-Up",
...
tau_hat_it = Y_it − Y0_hat_it.
This procedure ensures that treatment effects are identified only from comparisons with untreated observations, thereby avoiding the “already-treated as control” problem discussed in Part (c).
...
# drop observations where tau_hat is missing (these units never appear untreated in Ω0)
...
# --- Ensure required objects exist (helps when knitting) ---
if (!exists("att_hand")) {
  stop("Object 'att_hand' not found. Run Step E.1 (hand ATT regression) before this table chunk.")
}
if (!exists("att_pkg")) {
  stop("Object 'att_pkg' not found. Run Step E.2 (did_imputation) before this table chunk.")
}

# --- Extract estimates and standard errors ---
# Hand (naive) ATT and SE from the regression in Step E.1
hand_est <- as.numeric(coef(att_hand)["treated_it"])
hand_se  <- as.numeric(se(att_hand)["treated_it"])

# Package ATT and SE from the tibble returned by did_imputation in Step E.2
pkg_est <- as.numeric(att_pkg$estimate[att_pkg$term == "treat"])
pkg_se  <- as.numeric(att_pkg$std.error[att_pkg$term == "treat"])

# Sanity check: confirm extraction worked
cat("Extracted values:\n")
print(data.frame(
  hand_est = hand_est, hand_se = hand_se,
  pkg_est  = pkg_est,  pkg_se  = pkg_se
))

# --- Build a clean numeric table first (safe column names) ---
att_tbl <- data.frame(
  Method    = c("Hand imputation (naive SE)", "`did_imputation` (correct SE)"),
  ATT       = c(hand_est, pkg_est),
  Std_Error = c(hand_se,  pkg_se),
  check.names = FALSE
)
...
kable(
  att_tbl_pretty,
  caption  = "Table 2. ATT: Hand vs Package Imputation",
  align    = c("l", "c", "c"),
  booktabs = TRUE
) %>%
  kable_styling(
...
Table Z reports the average treatment effect on the treated (ATT) estimated using both the hand implementation of the imputation estimator and the did_imputation package. Both methods produce the same point estimate (ATT = 0.0917), which confirms that they implement the same imputation logic: untreated outcomes are first estimated using only untreated observations, and treatment effects are then constructed as tau_hat_it = Y_it − Y0_hat_it. This equivalence of point estimates is exactly what is predicted by the imputation framework of Borusyak et al. (2024) and by the exposition in Roth et al. (2023).
